...
const AddProduct = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(PRODUCT_API_URL, { method: "POST", body: submitData });
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#f0f2f5", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
        <div style={{ textAlign: "center" }}>
          ...
          <p style={{ marginTop: 10, color: "#6b7280", fontSize: 14 }}>Loading…</p>
        </div>
              ...
              <div style={{ marginBottom: 4 }}>
                ...
                <label
                  style={{
                    ...
                    borderRadius: 10, padding: "13px 16px", cursor: imageFiles.length >= 5 ? "not-allowed" : "pointer",
                    background: imageFiles.length > 0 ? "#f9f9f9" : "#fafafa",
                    transition: "all .15s", opacity: imageFiles.length >= 5 ? 0.5 : 1,
                  }}
                >
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={imageFiles.length > 0 ? "#1a1a1a" : "#9ca3af"} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
